...
def fetch_spectrum(usi: str) -> Dict[str, Any]:
    """Fetch spectrum data from PeptideAtlas API for a given USI."""
    base_url = "https://peptideatlas.org/api/proxi/v0.1/spectra"
    params = {
        'resultType': 'full',
        'usi': usi
    }
...
def process_usi_file(input_file: str, output_file: str = 'pa_mass_differences.txt', 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Filter USIs based on mass difference from PeptideAtlas API')
    parser.add_argument('input_file', help='Path to file containing USI identifiers')
    parser.add_argument('--output', default='pa_mass_differences.txt', help='Output file path')
